...
# 2. Kafka 为何需要 Hight Available 

## 2.1 为何需要Replication(拷贝)
...
- 如果Producer使用同步模式则Producer会在尝试重新发送message.send.max.retries（默认值为3）次后抛出Exception，用户可以选择停止发送后续数据也可选择继续发送。而前者会造成数据的阻塞，后者会造成本应发往该Broker的数据的丢失。
...
## 2.2 为何需要Leader Election(选举 Leader)
...
# 3. Kafka (HA)Hight Available 设计解析
...
![Kafka Replication的数据流](http://om4wzu26e.bkt.clouddn.com/Kafka%20Replication%E7%9A%84%E6%95%B0%E6%8D%AE%E6%B5%81.png)
...
**Leader会跟踪与其保持同步的Replica列表，该列表称为ISR（即in-sync Replica）**。如果一个Follower宕机，或者落后太多，Leader将把它从ISR中移除。这里所描述的“落后太多”指Follower复制的消息落后于Leader后的条数超过预定值（该值可在`$KAFKA_HOME/config/server.properties`中通过`replica.lag.max.messages`配置，其默认值是`4000`）或者Follower超过一定时间（该值可在`$KAFKA_HOME/config/server.properties`中通过`replica.lag.time.max.ms`来配置，其默认值是`10000`）未向Leader发送fetch请求。。
...
## 3.3 HA相关Zookeeper结构
主要: （本节所示Zookeeper结构中，实线框代表路径名是固定的，而虚线框代表路径名与业务相关）


### 3.3.1 admin
admin （该目录下znode只有在有相关操作时才会存在，操作结束时会将其删除）
![admin](http://om4wzu26e.bkt.clouddn.com/admin.png)
...
![broker](http://om4wzu26e.bkt.clouddn.com/brokers.png)
...
![Broker failover顺序图](http://om4wzu26e.bkt.clouddn.com/broker%20failover%E9%A1%BA%E5%BA%8F%E5%9B%BE.png)

## 参考
1. [Kafka High Availability](http://www.infoq.com/cn/articles/kafka-analysis-part-1?utm_source=infoq&utm_campaign=user_page&utm_medium=link)
